...
const Home: React.FC = () => {
  ...
  const statusCount = (label: string) => {
    ...
    return Object.entries(publicStats?.by_status || {}).reduce(
      (total, [key, value]) =>
        canonicalStatus(key).toLowerCase() === target
  ...
  const dashboardStats = useMemo(() => {
    if (!publicStats) return fallbackDashboardStats;
    return [
      { label: "Ongoing Projects", value: String(statusCount("Ongoing")) },
      { label: "Completed Projects", value: String(statusCount("Completed")) },
      {
        label: "Total Investment",
        value: formatMoneyCompact(publicStats.total_budget),
      },
      {
        label: "Agencies Covered",
        value: String(Object.keys(publicStats.by_agency || {}).length),
      },
    ];
  }, [publicStats]);

  const projectStatusData = useMemo(() => {
    if (!publicStats) return fallbackProjectStatusData;
    ...
    return Object.entries(publicStats.by_status || {})
      .map(([status, count]) => ({
  ...
  const investmentData = useMemo(() => {
    if (!publicStats) return fallbackInvestmentData;
    const entries = Object.entries(publicStats.by_agency || {})
  ...
  const timelineData = useMemo(() => {
    ...
    return years.length
      ? years.map((year) => ({
          quarter: String(year),
          projects: Number(byYear[String(year)] || 0),
        }))
  ...
  const statusTotal = useMemo(
    () =>
      Math.max(
        1,
        projectStatusData.reduce((sum, item) => sum + Number(item.count || 0), 0)
      ),
    [projectStatusData]
  );
  ...
  useEffect(() => {
    ...
    const loadPublicStats = async () => {
      try {
        setPublicStatsError("");
        const stats = await getPublicProjectsStats();
    ...
    loadPublicStats();
    const timer = window.setInterval(loadPublicStats, 60000);
    return () => {
      cancelled = true;
