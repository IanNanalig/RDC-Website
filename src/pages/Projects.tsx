...
const Projects: React.FC = () => {
  ...
  const displayProjects = useMemo(() => {
    if (selectedCity) {
      return filtered.filter((p) => (p.lgus?.length ? p.lgus.includes(selectedCity) : p.lgu === selectedCity));
    } else if (municipalityFilter === "Unspecified") {
      return filtered.filter((p) => !(p.lgus?.length) && !p.lgu);
    } else if (municipalityFilter !== "all") {
      return filtered.filter((p) =>
        p.lgus?.length ? p.lgus.includes(municipalityFilter) : p.lgu === municipalityFilter,
      );
    }
    return filtered;
  ...
  const sortedTableProjects = useMemo(() => {
    ...
    sorted.sort((a, b) => {
      ...
      if (sortColumn === "lgu") {
        aVal = a.lgus?.length ? a.lgus.join(", ") : a.lgu || "Unspecified";
        bVal = b.lgus?.length ? b.lgus.join(", ") : b.lgu || "Unspecified";
      }
                      ...
                      {sortedTableProjects.length > 0 ? (
                        sortedTableProjects.map((p, idx) => (
                          ...
                          >
                            ...
                            <td className="px-4 py-3 text-sm text-gray-700">
                              {p.lgus?.length ? p.lgus.join(", ") : p.lgu || "Unspecified"}
                            </td>
                          </tr>
                        ))
                      ) : (
                  ...
                  <div className="flex items-center justify-between px-5 py-4 border-b">
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-slate-500 mt-0.5">
                        ...
                        {(detailProject.lgus?.length || detailProject.lgu)
                          ? `• ${(detailProject.lgus?.length ? detailProject.lgus : [detailProject.lgu]).filter(Boolean).join(", ")}`
                          : ""}
                      </p>
                    </div>
                    <button
