...
const SimplifiedProjectSubmission: React.FC = () => {
  ...
  const [aiPriorityLevel, setAiPriorityLevel] = useState("");
  ...
  useEffect(() => {
    const loadExisting = async () => {
      ...
      try {
        ...
        setAiPriorityScore(
          typeof data?.ai_priority_score === "number" ? data.ai_priority_score : null,
        );
        setAiPriorityLevel(String(data?.ai_priority_level || ""));
        setAiAnalysisNotes(String(data?.ai_analysis_notes || ""));
        setAiAnalyzedAt(String(data?.ai_analyzed_at || ""));
        if (data?.updated_at) {
  ...
  const runAiScorer = async () => {
    if (!id || !(isValidator || isAdmin)) return;
    setAiScoring(true);
    try {
      const base = isValidator ? "validator" : "admin";
      const data = await api.post(`${base}/projects/${id}/ai-score/`, {});
      setAiScoreReview(data);
      setAiPriorityScore(typeof data?.score === "number" ? data.score : Number(data?.score || 0));
      setAiPriorityLevel(String(data?.grade || ""));
      setAiAnalysisNotes(String(data?.justification || data?.summary || ""));
      setAiAnalyzedAt(String(data?.created_at || ""));
    } catch (error) {
      console.error("Failed to run AI scorer:", error);
  ...
  return (
    ...
    >
      ...
      {(isValidator || isAdmin) && id && (
        <div className="portal-card p-4 mb-3 border-emerald-200 bg-emerald-50">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div>
              <h2 className="font-semibold text-emerald-950">AI Priority Scorer</h2>
              <p className="text-xs text-emerald-800 mt-1">
                Score:{" "}
                <span className="font-semibold">
                  {aiPriorityScore === null ? "Not scored" : aiPriorityScore}
                </span>
                {aiPriorityLevel ? ` | ${aiPriorityLevel}` : ""}
                {aiAnalyzedAt ? ` | ${new Date(aiAnalyzedAt).toLocaleString()}` : ""}
              </p>
            </div>
            <button
              type="button"
              className="portal-btn portal-btn-primary bg-emerald-700 hover:bg-emerald-800 disabled:opacity-60"
              onClick={runAiScorer}
              disabled={aiScoring}
            >
              {aiScoring ? "Scoring..." : "Run AI Scorer"}
            </button>
          </div>
          {(aiScoreReview?.justification || aiAnalysisNotes) && (
            <p className="mt-3 text-sm text-emerald-950 whitespace-pre-wrap">
              {aiScoreReview?.justification || aiAnalysisNotes}
            </p>
          )}
          {aiScoreReview?.summary ? (
            <p className="mt-2 text-sm text-emerald-900 whitespace-pre-wrap">{aiScoreReview.summary}</p>
          ) : null}
