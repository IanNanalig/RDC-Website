...
const UserManagement = () => {
  ...
  const formatFieldName = (field: string) => {
    ...
    const labels: Record<string, string> = {
      ...
      ludipIncluded: "LUDIP Included",
      ifpsIncluded: "IFPs Included",
      pcbIncluded: "Part of Convergence Program (PCB)",
    ...
    if (normalized.startsWith("fundingRequirementByYear.")) {
      const key = normalized.split(".")[1] || "";
      return `Funding Requirement ${key === "2022_prior" ? "2022 & Prior" : key}`;
    }
    if (normalized.startsWith("actualFundingByYear.")) {
      const key = normalized.split(".")[1] || "";
      return `Actual Funding ${key === "2022_prior" ? "2022 & Prior" : key}`;
    }
    return labels[normalized] || formatDetailLabel(normalized);
  };

  const getActivityChanges = (item: ActivityItem): ActivityChange[] => {
    const raw = item.details?.changes;
    ...
    return raw
      .map((entry) => {
        ...
        return {
          ...
          before: formatDetailValue(obj.before),
          after: formatDetailValue(obj.after),
        };
      })
      .filter((entry): entry is ActivityChange => Boolean(entry?.field));
  };
  ...
  const buildDetailEntries = (item: ActivityItem): Array<[string, string]> => {
    ...
    Object.entries(details)
      .filter(([key, value]) => key !== "changes" && !used.has(key) && value !== undefined && value !== null && value !== "")
      .forEach(([key, value]) => {
  ...
  const getErrorDetail = (err: unknown, fallback: string) => {
    if (err instanceof Error && err.message) {
      try {
        ...
        const looksLikeMarkup = /<\/?[a-z][\s\S]*>/i.test(message);
        const looksLikeTraceback = /traceback|exception|syntaxerror|typeerror|referenceerror/i.test(message);
        if (!looksLikeMarkup && !looksLikeTraceback && message.length < 220) {
          return message;
        }
      }
    }
  ...
  const createAccount = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setNotice(getErrorDetail(err, "Failed to create account. Please check the email and user type."));
    } finally {
      ...
      {activeTab === "create-account" && (
        <div className="portal-card">
          ...
          <form onSubmit={createAccount} className="portal-card-body grid grid-cols-1 xl:grid-cols-2 gap-3">
            ...
            <button
              type="submit"
              className={`portal-btn portal-btn-primary xl:col-span-2 ${creatingAccount ? "opacity-70 cursor-not-allowed" : ""}`}
              disabled={creatingAccount}
            >
              {creatingAccount ? "Creating Account..." : "Create User"}
            </button>
          </form>
        </div>
      )}
                                  ...
                                  <table className="min-w-full border-collapse">
                                    ...
                                    <tbody>
                                      {changes.map((change, index) => (
                                        <tr key={`${item.id}-${change.field}-${index}`} className="border-b border-slate-200 last:border-b-0">
                                          <td className="px-2 py-1 font-semibold whitespace-nowrap">{formatFieldName(change.field)}</td>
                                          <td className="px-2 py-1 min-w-48 whitespace-normal break-words align-top">{change.before}</td>
                                          <td className="px-2 py-1 min-w-48 whitespace-normal break-words align-top">{change.after}</td>
                                        </tr>
                                      ))}
                                    </tbody>
                                  </table>
