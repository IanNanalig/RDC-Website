...
function getProjectCoordinates(project: Project): [number, number] | null {
  const city = project.lgus?.find((lgu) => ncrCityCenters[lgu]) || project.lgu;
  if (city && ncrCityCenters[city]) {
    return ncrCityCenters[city];
  }
  return null;
}
...
const VoronoiBlobMap: React.FC<VoronoiBlobMapProps> = ({
  ...
  const groups = useMemo(() => {
    ...
    projects.forEach((p) => {
      const cities = p.lgus?.length ? p.lgus : p.lgu ? [p.lgu] : [];
      cities.forEach((city) => {
