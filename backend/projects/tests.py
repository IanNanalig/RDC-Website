...
from .models import PasswordSetupToken, Project, SystemSetting, User, UserActivity
...
class PortalWorkflowTests(APITestCase):
    ...
    def test_login_updates_stale_temporary_username_from_profile_name(self):
        user = User.objects.create_user(
            username="danny345678910df21",
            email="danny345678910df21@gmail.com",
            password="ValidPass123!",
            role="staff",
            full_name="Danny Santos",
            agency="DHSUD",
        ...
        res = self.client.post(
            "/api/auth/login/",
            {"email": "danny345678910df21@gmail.com", "password": "ValidPass123!"},
            format="json",
        )

        self.assertEqual(res.status_code, status.HTTP_200_OK)
        self.assertEqual(res.data["user"]["username"], "Danny Santos")
        user.refresh_from_db()
        self.assertEqual(user.username, "Danny Santos")

    def test_password_setup_updates_username_used_by_login_and_admin_users(self):
        user = User(
        ...
        token = PasswordSetupToken.objects.create(
            user=user,
            token="setup-token-for-username-test",
        ...
        login_res = self.client.post(
            "/api/auth/login/",
            {"email": "new.employee@example.com", "password": "ValidPass123!"},
            format="json",
        )
        self.assertEqual(login_res.status_code, status.HTTP_200_OK)
        self.assertEqual(login_res.data["user"]["username"], "New Employee")

        self._as(self.admin)
        users_res = self.client.get("/api/admin/users/")
        self.assertEqual(users_res.status_code, status.HTTP_200_OK)
        created = next((u for u in users_res.data if u["email"] == "new.employee@example.com"), None)
        self.assertIsNotNone(created)
        self.assertEqual(created["username"], "New Employee")
