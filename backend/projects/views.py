...
from .serializers import (
    ...
    AIProjectScoreReviewSerializer,
...
from .ai_scorer import score_project
...
PUBLIC_PROJECTS_CACHE_CONTROL = f"public, max-age={PUBLIC_PROJECTS_CACHE_TTL_SECONDS}"
...
def _build_unique_profile_username(full_name: str, email: str = "", exclude_user_id=None):
    base = re.sub(r"\s+", " ", (full_name or "").strip())
    if not base:
        base = _build_unique_username(email)
    base = base[:145].strip() or "user"
    candidate = base
    idx = 1
    qs = User.objects.all()
    if exclude_user_id:
...
class BaseProjectViewSet(viewsets.ModelViewSet):
    ...
    def ai_score(self, request, pk=None):
        ...
        role = getattr(request.user, "role", "")
        if role not in ("admin", "validator"):
            raise PermissionDenied("Only admin/validator can run AI scoring.")

        result = score_project(
            project.title,
            getattr(project, "description", "") or "",
            project.profile_data if isinstance(project.profile_data, dict) else {},
        )
        score = float(result.get("score") or 0)
        grade = str(result.get("grade") or "Medium Priority")
        ...
        review = AIProjectScoreReview.objects.create(
            project=project,
            ...
            negative_matches=result.get("negative_matches") or [],
        ...
        project.ai_priority_score = score
        project.ai_priority_level = grade
        project.ai_analysis_notes = justification or summary
        project.ai_analyzed_at = timezone.now()
        project.save(
            update_fields=[
                "ai_priority_score",
                "ai_priority_level",
        ...
        _log_activity(
            ...
            "project_update",
            project,
            {"ai_score": score, "ai_grade": grade, "action": "ai_score"},
        )
        return Response(AIProjectScoreReviewSerializer(review).data)
    ...
    def score(self, request, pk=None):
        ...
        role = getattr(request.user, "role", "")
        if role not in ("admin", "validator"):
            raise PermissionDenied("Only validators and admins can score projects")
        ...
        _log_activity(
            ...
            "project_update",
            project,
            {"ai_score": score, "ai_grade": grade, "action": "score"},
        )
        return Response(
            {
                **result,
    ...
    def score_history(self, request, pk=None):
        ...
        role = getattr(request.user, "role", "")
        if role not in ("admin", "validator"):
            raise PermissionDenied("Only validators and admins can view score history")
        ...
        reviews = project.ai_score_reviews.select_related("reviewed_by").all()[:limit]
        data = []
        for review in reviews:
            ...
            if reviewer:
                reviewer_name = (
                    getattr(reviewer, "full_name", "").strip()
                    or reviewer.get_full_name()
                    or reviewer.username
                )
            raw_result = review.raw_result if isinstance(review.raw_result, dict) else {}
            data.append(
                {
                    "id": review.id,
                    "score": review.score,
                    ...
                    "negative_matches": review.negative_matches,
...
class LoginView(APIView):
    ...
    def post(self, request):
        ...
        if full_name and user.username != full_name:
            user.username = _build_unique_profile_username(full_name, user.email, exclude_user_id=user.id)
...
class SetupPasswordView(APIView):
    ...
    def post(self, request):
        ...
        user.username = _build_unique_profile_username(user.full_name, user.email, exclude_user_id=user.id)
...
class PublicProjectsViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        if cached is not None:
            ...
            resp["Cache-Control"] = PUBLIC_PROJECTS_CACHE_CONTROL
            return resp
        ...
        if lgu_filter and lgu_filter.lower() != "all":
            ...
            if lgu_filter == "Unspecified":
                filtered_payload = [p for p in all_payload if not p.get("lgus") and not p.get("lgu")]
            else:
                filtered_payload = [
                    p
                    for p in all_payload
                    if lgu_filter in (p.get("lgus") or ([p.get("lgu")] if p.get("lgu") else []))
                ]
...
class PublicProjectsStatsView(APIView):
    ...
    def get(self, request):
        ...
        for p in projects:
            ...
            project_updated_at = getattr(p, "updated_at", None)
            if project_updated_at and (
                last_updated_at is None or project_updated_at > last_updated_at
            ):
