...
def derive_ncr_lgus(location_text: str | None) -> list[str]:
    """
    Extract all NCR LGUs mentioned in human-entered location text.
    Keeps canonical order from NCR_LGU_CANONICAL and returns an empty list when none are recognized.
    """
    raw = str(location_text or "").strip()
    if not raw:
        return []
    ...
    for canonical, variants in NCR_LGU_CANONICAL.items():
        matched = False
        for v in variants:
            if v in candidates or (v and (v in lowered or v in plain)):
                matched = True
                break
        if matched:
