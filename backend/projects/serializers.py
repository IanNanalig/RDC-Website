...
from .models import AIProjectScoreReview, AccessRequest, PasswordResetRequest, Project, ProjectComment, User, UserActivity
...
class ProjectSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        if not validated_data.get("agency"):
            request = self.context.get("request")
            user_agency = getattr(getattr(request, "user", None), "agency", "")
            validated_data["agency"] = user_agency
...
class AIProjectScoreReviewSerializer(serializers.ModelSerializer):
    reviewed_by_name = serializers.SerializerMethodField()

    class Meta:
        model = AIProjectScoreReview
        fields = [
            "id",
            "project",
            "reviewed_by",
    ...
    def get_reviewed_by_name(self, obj):
        ...
        if not user:
            return ""
        return getattr(user, "full_name", "").strip() or user.get_full_name() or user.username
